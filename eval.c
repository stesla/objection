...
static inline ref_t eval_args(ref_t closure, ref_t env, ref_t args) {
  return isnil(args) ? NIL : cons(eval(closure, env, car(args)), eval_args(closure, env, cdr(args)));
}

static ref_t eval_list(ref_t closure, ref_t env, ref_t expr) {
...
ref_t apply(ref_t closure, ref_t env, ref_t func, ref_t args) {
...
ref_t macroexpand1(ref_t closure, ref_t env, ref_t expr) {
  if (!iscons(expr))
    return expr;
...
ref_t macroexpand(ref_t closure, ref_t env, ref_t expr) {
  ref_t expr1;
...
ref_t eval(ref_t closure, ref_t env, ref_t expr) {
