...
static ref_t fn_add(ref_t closure, ref_t env, ref_t func, ref_t args) {
  return binary_integer_op(integer_add, args);
}

static ref_t fn_apply(ref_t closure, ref_t env, ref_t func, ref_t args) {
  ref_t func1 = check_function(car(args)), args1 = check_list(cadr(args));
  return apply(closure, env, func1, args1);
}

static ref_t fn_car(ref_t closure, ref_t env, ref_t func, ref_t args) {
  return car(check_list(car(args)));
}

static ref_t fn_cdr(ref_t closure, ref_t env, ref_t func, ref_t args) {
  return cdr(check_list(car(args)));
}

static ref_t fn_cons(ref_t closure, ref_t env, ref_t func, ref_t args) {
  return cons(car(args), cadr(args));
}

static ref_t fn_div(ref_t closure, ref_t env, ref_t func, ref_t args) {
  return binary_integer_op(integer_div, args);
}

static ref_t fn_eq(ref_t closure, ref_t env, ref_t func, ref_t args) {
  return (car(args) == cadr(args)) ? TRUE : NIL;
}

static ref_t special_do(ref_t closure, ref_t env, ref_t func, ref_t args);
static ref_t fn_fn(ref_t closure, ref_t env, ref_t func, ref_t args) {
...
static ref_t fn_function(ref_t closure, ref_t env, ref_t func, ref_t args) {
  ref_t func1 = car(args);
  return issymbol(func1) ? get_function(func1) : check_function(func1);
}

static ref_t fn_list(ref_t closure, ref_t env, ref_t func, ref_t args) {
  return args;
}

static ref_t fn_macro(ref_t closure, ref_t env, ref_t func, ref_t args) {
  return set_type_macro(check_function(car(args)));
}

static ref_t fn_macroexpand(ref_t closure, ref_t env, ref_t func, ref_t args) {
  return macroexpand(closure, env, car(args));
}

static ref_t fn_macroexpand1(ref_t closure, ref_t env, ref_t func, ref_t args) {
  return macroexpand1(closure, env, car(args));
}

static ref_t fn_mul(ref_t closure, ref_t env, ref_t func, ref_t args) {
  return binary_integer_op(integer_mul, args);
}

static ref_t fn_set_function(ref_t closure, ref_t env, ref_t func, ref_t args) {
  ref_t symbol = check_symbol(car(args)), fn = check_function(cadr(args));
  set_function(symbol, fn);
  return fn;
}

static ref_t fn_set_value(ref_t closure, ref_t env, ref_t func, ref_t args) {
  ref_t symbol = check_symbol(car(args)), value = cadr(args);;
  set_value(symbol, value);
  return value;
}

static ref_t fn_sub(ref_t closure, ref_t env, ref_t func, ref_t args) {
  return binary_integer_op(integer_sub, args);
}

static ref_t special_fn(ref_t closure, ref_t env, ref_t func, ref_t args);

static ref_t macro_defn(ref_t closure, ref_t env, ref_t func, ref_t args) {
...
static ref_t macro_defmacro(ref_t closure, ref_t env, ref_t func, ref_t args) {
  return cons(intern(env, "set-function"),
              cons(cons(intern(env, "quote"), cons(check_symbol(car(args)), NIL)),
...
static ref_t special_if(ref_t closure, ref_t env, ref_t func, ref_t args) {
...
static ref_t special_quote(ref_t closure, ref_t env, ref_t func, ref_t args) {
  return car(args);
}

static inline void intern_function(ref_t env, const char *name, fn_t impl, size_t arity, bool rest) {
  set_function(intern(env, name), function(impl, NIL, arity, rest));
}

static inline void intern_macro(ref_t env, const char *name, fn_t impl, size_t arity, bool rest) {
  set_function(intern(env, name), set_type_macro(function(impl, NIL, arity, rest)));
}

static inline void intern_special_form(ref_t env, const char *name, fn_t impl, size_t arity, bool rest) {
